...
class RotaSchema {
  static listar = z.object({
    ...
    rota: z.array(z.string({
      message: "A rota precisa ser definida como uma string/texto."
    }).refine((val) => !/^\d+$/.test(val), {
      message: "A rota precisa ser em palavras e não em números.",
    })).refine((val) => val.every((v) => v.startsWith("/")), {
      message: "A rota deve começar com '/'",
    }).optional(),
  });
  static criar = z.object({
    ...
    rota: z.array(z.string({
      ...
      message: "A rota precisa ser em palavras e não em números.",
    })).min(1, "A rota é obrigatória e deve conter pelo menos 1 caractere.").refine((val) => val.every((v) => v.startsWith("/")), {
      message: "A rota deve começar com '/'",
    }),
  });
